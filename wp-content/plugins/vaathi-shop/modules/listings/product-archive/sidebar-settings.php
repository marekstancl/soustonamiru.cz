...
if( !class_exists( 'Vaathi_Woo_Listing_Fw_Archive_Sidebar_Settings' ) ) {

    class Vaathi_Woo_Listing_Fw_Archive_Sidebar_Settings {
        ...
        function __construct($shop_custom_options) {
            ...
            $this->global_layout  = vaathi_customizer_settings('global_sidebar_layout');
            $this->global_sidebar = vaathi_customizer_settings('global_sidebar');
            $this->hide_standard_sidebar = vaathi_customizer_settings('hide_standard_sidebar');
        ...
        function frontend() {
            add_filter('vaathi_primary_classes', array( $this, 'primary_classes' ), 50 );
            add_filter('vaathi_secondary_classes', array( $this, 'secondary_classes' ), 50 );
            add_filter('vaathi_active_sidebars', array( $this, 'active_sidebars' ), 50 );
            add_filter('vaathi_woo_loop_column_class', array( $this, 'woo_loop_column_class' ), 50, 2 );
        }
                ...
                if($this->primary_class != 'content-full-width') {
                    if($this->cs_custom_layout == 'global-sidebar-layout' || $this->cs_custom_layout == '') {
                        ...
                        if($this->hide_standard_sidebar) {
                            unset($sidebars[array_search('vaathi-standard-sidebar-1', $sidebars)]);
                        }
                    } else {
