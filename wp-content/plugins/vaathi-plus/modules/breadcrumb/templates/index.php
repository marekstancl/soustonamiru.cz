...
if( !class_exists( 'VaathiPlusBCTemplate' ) ) {
    class VaathiPlusBCTemplate {
        ...
        function load_frontend() {
            add_filter( 'vaathi_breadcrumb_params', array( $this, 'register_breadcrumb_params' ) );
            add_filter( 'vaathi_breadcrumb_get_template_part', array( $this, 'register_template' ), 10, 2 );

            add_action( 'vaathi_after_main_css', array( $this, 'enqueue_assets' ) );

            add_filter( 'vaathi_header_wrapper_classes', array( $this, 'register_header_class' ), 20, 1 );
        }

        function register_header_class($classes) {

            $breadcrumb_position = vaathi_customizer_settings('breadcrumb_position');
            $dark_bg_breadcrumb = vaathi_customizer_settings('enable_dark_bg_breadcrumb');
        ...
        function enqueue_assets() {
            wp_enqueue_style( 'site-breadcrumb', VAATHI_PLUS_DIR_URL . 'modules/breadcrumb/assets/css/breadcrumb.css', VAATHI_PLUS_VERSION );
        }
        ...
        function base_meta_style( $meta ) {
            ...
            if( !empty( $bg_css ) && empty( $enable_overlay ) ) {
                ...
                $style .= vaathi_customizer_dynamic_style( '.main-title-section-wrapper.overlay-wrapper.dark-bg-breadcrumb > .main-title-section-bg, .main-title-section-wrapper.overlay-wrapper > .main-title-section-bg, .main-title-section-wrapper.dark-bg-breadcrumb > .main-title-section-bg, .main-title-section-wrapper > .main-title-section-bg', $bg_css );
                ...
                if( !empty( $overlay_color ) || !empty( $overlay_gradient_color ) ) {
                    $bg_css = vaathi_customizer_bg_settings( array( 'background-color' => $overlay_color, 'gradient-background-color' => $overlay_gradient_color, 'breadcrumb_overlay_bg_color' => true ) );
            ...
            if( !empty( $style ) ){
                wp_register_style( 'vaathi-breadcrumb-inline', '', array (), VAATHI_PLUS_VERSION, 'all' );
                wp_enqueue_style( 'vaathi-breadcrumb-inline' );
                wp_add_inline_style( 'vaathi-breadcrumb-inline', $style );
            }
        }

        function register_breadcrumb_params() {

            $enable_delimiter = vaathi_customizer_settings( 'change_breadcrumb_delimiter' );
            $delimiter        = vaathi_customizer_settings( 'breadcrumb_delimiter' );

            $delimiter = ( $enable_delimiter ) ? '<span class="'.esc_attr($delimiter).'"></span>' : '<span class="breadcrumb-default-delimiter"></span>';

            $wrapper_class    = array();
            $enable_darkbg    = vaathi_customizer_settings( 'enable_dark_bg_breadcrumb' );
            $breadcrumb_style = vaathi_customizer_settings( 'breadcrumb_style' );
            ...
            $bc_overlay_bg_color = vaathi_customizer_settings( 'breadcrumb_overlay_bg_color' );
            if($bc_overlay_bg_color) {
                $wrapper_class[] = 'overlay-wrapper';
            }

            $hide_content    = vaathi_customizer_settings( 'hide_content' );
            if( $hide_content ) {
                $wrapper_class[] = 'hide-content';
            }

            $params = array(
                'home'             => esc_html__( 'Home', 'vaathi-plus' ),
                'home_link'        => home_url('/'),
                'delimiter'        => $delimiter,
        ...
        function register_meta_params( $post_id ) {
            ...
            $post_meta = get_post_meta( $post_id, '_vaathi_breadcrumb_settings', true );
        ...
        function register_template( $args, $post_id ) {
            ...
            if( empty($bc_meta) || ( array_key_exists( 'layout', $bc_meta ) && $bc_meta['layout'] != 'disable' ) ) {
                ...
                if( array_key_exists( 'layout', $bc_meta ) && $bc_meta['layout'] == 'individual-option' ) {
                    ...
                    $enable_bc = vaathi_customizer_settings( 'enable_breadcrumb' );
                    if( ! $enable_bc ) {
                        return;
                    }
                }

                $bc_source = vaathi_customizer_settings( 'breadcrumb_source' );

                switch( $bc_source ):

                    case 'default':
                    default:
                        vaathi_template_part( 'breadcrumb', 'templates/default/title-content', '', $template_args );
                    break;
