<?php
if ( ! class_exists( 'VaathiPlusFrontendMenuWalker' ) ) {

	class VaathiPlusFrontendMenuWalker {
        ...
        function frontend() {
        	...
        	add_action( 'vaathi_before_enqueue_js', array( $this, 'enqueue_js_assets' ) );
        }

		function enqueue_css_assets() {
			wp_enqueue_style( 'dtplugin-nav-menu-animations', VAATHI_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu-animations.css', false, VAATHI_PLUS_VERSION, 'all');
			wp_enqueue_style( 'dtplugin-nav-menu', VAATHI_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu.css', false, VAATHI_PLUS_VERSION, 'all');
		}

		function enqueue_js_assets() {
			wp_enqueue_script( 'dtplugin-mega-menu', VAATHI_PLUS_DIR_URL . 'modules/menu/assets/js/mega-menu.js', array(), VAATHI_PLUS_VERSION, true );
		}
	}
}

VaathiPlusFrontendMenuWalker::instance();

if( ! class_exists( 'Vaathi_Walker_Nav_Menu' ) ) {

	class Vaathi_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			$item_output  = $args_object->before;
			$item_output .= '<a' . vaathi_html_output($attributes) . '>';
				...
				if( !empty( $item->icon ) || !empty( $item->image ) ) {
					...
					if( !empty( $item->image ) ) {
						$item_output .= '<i class="menu-item-image">';
							$item_output .= '<img src="'.esc_url( $item->image ).'" alt="'.esc_attr( 'Image', 'vaathi-plus' ).'"/>';
						$item_output .= '</i>';
					}
				}
